...
# Global variable to configure the asynchronous mode
# Options: "threading", "callback", "coroutine"
...
async def handle_client_coroutine(reader, writer, routes={}):
    ...
    :param routes (dict): Route handlers dictionary.
    """
...
async def async_server(ip="0.0.0.0", port=7000, routes={}):
    """
    Start an async server using asyncio.start_server.
    ...
    # Create handler with captured routes
...
def run_backend(ip, port, routes):
    ...
    print("[Backend] Non-blocking mode: {}".format(mode_async))
    ...
    try:
        ...
        if mode_async == "callback":
            ...
            print("[Backend] Using callback (selector) mode")
            ...
            print("[Backend] Using multi-thread mode")
